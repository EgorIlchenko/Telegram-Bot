...
def genre_search(message: Message) -> None:
    user_id = message.from_user.id
    if User.get_or_none(User.user_id == user_id) is None:
        bot.reply_to(message, "Вы не зарегистрированы. Напишите /start")
        return

    bot.set_state(message.from_user.id, SearchInfo.film_genre, message.chat.id)
    bot.send_message(message.from_user.id, 'Введите жанр для поиска _(Пример: драма, ужасы и тд.)_',
                     parse_mode='Markdown')
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['user_id'] = user_id


@bot.message_handler(state=SearchInfo.film_genre)
def get_film_genre(message: Message) -> None:
    bot.send_message(message.from_user.id, 'Сколько фильмов показать?')
    bot.set_state(message.from_user.id, SearchInfo.number_options_genre, message.chat.id)

    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['film_genre'] = tuple(message.text.split(', '))


@bot.message_handler(state=SearchInfo.number_options_genre)
def get_number_options_genre(message: Message) -> None:
    if message.text.isdigit():
        bot.send_message(message.from_user.id, 'Вот, что смог найти для Вас:')

        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            data['number_options_genre'] = message.text

            result_req = request_info.search_film_genre(film_genre=data['film_genre'],
                                                        number_films=data['number_options_genre'])
