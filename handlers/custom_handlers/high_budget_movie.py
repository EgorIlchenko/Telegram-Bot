...
def high_budget_search(message: Message) -> None:
    ...
    bot.set_state(message.from_user.id, SearchInfo.number_options_high_budget, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['user_id'] = user_id
        data['film_high_budget'] = f'{random.randrange(50000000, 800000000, 10000000)}'


@bot.message_handler(state=SearchInfo.number_options_high_budget)
def get_number_options_high_budget(message: Message) -> None:
    if message.text.isdigit():
        bot.send_message(message.from_user.id, 'Вот, что смог найти для Вас:')

        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            data['number_options_high_budget'] = message.text

            result_req = request_info.search_film_high_budget(film_high_budget=data['film_high_budget'],
                                                              number_films=data['number_options_high_budget'])
